...
namespace msa { namespace control {
	...
	static Event *peek_event(Handle msa);
	static Event *pop_event(Handle msa);

	static void *edt_start(void *args);
	static void edt_run(Handle hdl);
	static Event *edt_poll_event_queue(Handle hdl);
	static void edt_interrupt_handler(Handle hdl);
	static void edt_spawn_handler(Handle hdl, Event *e);
	static void edt_dispatch_event(Handle hdl, Event *e);
	static void edit_dispose_handler_context(Handle hdl);
	...
	struct environment_type
	{
		...
		std::priority_queue<msa::event::Event *> event_queue;
	...
	extern int quit(Handle msa)
	{
		...
		pthread_mutex_destroy(&msa->queue_mutex);
	...
	static void edt_run(Handle hdl) {
		...
		Event *e = edt_poll_event_queue(hdl);
		if (e != NULL)
		{
		...
		{
			HandlerContext *ctx = hdl->interrupted_handlers.pop();
	...
	static void push_event(Handle msa, Event *e)
	{
